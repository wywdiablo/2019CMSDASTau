//----------------------Version 1.0-------------------------//
//Macro Produing shapes scaling for ZTT process Macro for Mu -> Tau Fake Rates study
//Author: Yiwen Wen
//DESY
//-----------------------------------------------------------//
...
#include "TColor.h"
#include <math.h>
void ProduceDatacardInputs_MuTauFR_m_vis_Scale_ZTT(
                                 TString varName = "m_vis_probetau_scale_up",
                                 TString uncertSuffix = "probetau_Up",
                                 ...
                                 )
{
	...
	TString samples[1] =
                    {
                        "NTuples_DYJetsToLL_M-50"// (0)Drell-Yan Z->TT(hadronic tau)ZTTmt
                    };

	float xsec[1] = {
                        6077.22,// (0)Drell-Yan Z->TT, ZTTmt
                    };
    TString suffix("");
    if(passProbe)
        suffix="_pass";
    ...
    for (int i=0; i<9; ++i) { // run over samples
        ...
        for (int iB=1; iB<=nBins; ++iB) {
            double x = histZTTmt[i]->GetBinContent(iB);
            double e = histZTTmt[i]->GetBinError(iB);
    ...
    {
        hist[0]->Add(hist[0],histZTTmt[iDY]);
    }
    ...
    TH1D * ZTT_mt = (TH1D*)hist[0]->Clone("ZTT_mt_"+uncertSuffix);
